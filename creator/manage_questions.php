...
// ตรวจสอบสิทธิ์เข้าถึงชุดข้อสอบ
if ($quiz_id > 0) {
    $owner_check = $conn->query("SELECT created_by FROM quizzes WHERE id = $quiz_id")->fetch_assoc();
    if (!$owner_check || $owner_check['created_by'] != $creator_id) {
        setAlert('คุณไม่มีสิทธิ์เข้าถึงชุดข้อสอบนี้', 'error');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create'])) {
    ...
    if ($owner_check && $owner_check['created_by'] == $creator_id) {
        ...
        setAlert('คุณไม่มีสิทธิ์เพิ่มคำถามในชุดข้อสอบนี้', 'error');
    }
...
if (isset($_GET['delete'])) {
    ...
    // ตรวจสอบว่าคำถามนี้อยู่ในชุดข้อสอบของตัวเองหรือไม่
    $check = $conn->query("SELECT q.quiz_id FROM questions q JOIN quizzes qz ON q.quiz_id = qz.id WHERE q.id = $question_id AND qz.created_by = $creator_id")->fetch_assoc();
    
    if ($check) {
        $conn->query("DELETE FROM questions WHERE id = $question_id");
        setAlert('ลบคำถามสำเร็จ!', 'success');
    } else {
        setAlert('คุณไม่มีสิทธิ์ลบคำถามนี้', 'error');
    }
    redirect("manage_questions.php?quiz_id=$quiz_id");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if ($check) {
        ...
        $stmt->bind_param("sssssssii", $question_text, $option_a, $option_b, $option_c, $option_d, $correct_answer, $explanation, $order_num, $question_id);
        
        if ($stmt->execute()) {
            setAlert('แก้ไขคำถามสำเร็จ!', 'success');
        }
    } else {
        setAlert('คุณไม่มีสิทธิ์แก้ไขคำถามนี้', 'error');
    }
    redirect("manage_questions.php?quiz_id=$quiz_id");
}

// ดึงชุดข้อสอบของตัวเองเท่านั้น
$quizzes = $conn->query("SELECT id, title FROM quizzes WHERE created_by = $creator_id ORDER BY title");

// ดึงข้อมูลชุดข้อสอบที่เลือก
$selected_quiz = null;
if ($quiz_id > 0) {
    $selected_quiz = $conn->query("SELECT * FROM quizzes WHERE id = $quiz_id AND created_by = $creator_id")->fetch_assoc();
...
if (isset($_GET['edit'])) {
    ...
    $edit_result = $conn->query("SELECT q.* FROM questions q JOIN quizzes qz ON q.quiz_id = qz.id WHERE q.id = $edit_id AND qz.created_by = $creator_id");
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sidebar-header h2 { font-size: 1.5em; margin-bottom: 5px; }
...
<body>
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="manage_quizzes.php" class="menu-item">📝 สร้างชุดข้อสอบ</a>
            <a href="manage_questions.php" class="menu-item active">❓ จัดการคำถาม</a>
    ...
    <div class="main-content">
        ...
        <div class="quiz-selector">
            <h3 style="margin-bottom: 10px;">📚 เลือกชุดข้อสอบของคุณ</h3>
